...
doOne <- function(i) {
	...
	# Recompute the P-value since truncreg did not do that correctly
	...
	if (!is.null(opt$result_var_name)) {
		if (is.null(..patterns)) ..patterns <<- unique(c(rownames(result)[grep(opt$result_var_pattern, rownames(result))], "sigma"));
