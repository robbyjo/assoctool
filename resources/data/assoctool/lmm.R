...
..lmFun <- match.fun(ifelse(opt$method == "lmm", "lmer", ifelse(opt$method == "glmm", "glmer", "nlmer")));
...
if (is.null(param_list[['control']])) {
	if (opt$method == "lmm") {
		#param_list$control <- lmerControl(optimizer="optimx", calc.derivs=FALSE, check.nobs.vs.rankZ = "ignore", check.nobs.vs.nlev="ignore", check.nobs.vs.nRE="ignore", optCtrl = list(method = "nlminb", starttests = FALSE, kkt = FALSE));
